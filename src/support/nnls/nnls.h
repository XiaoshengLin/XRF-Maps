...
namespace nsNNLS {
  ...
  private:                      
	  // computes / sets x0, g0, oldx, oldg, etc.
    int    initialize()
	{
		size_t n = A->ncols();
		x = new vector<_T>(n);
	...
	// embodies various termination criteria
	...
	// memory deallocation and friends
    int    cleanUp()
	{
		delete ax;
		delete oldx;
	...
	// compute both together to sav time
    void   computeObjGrad()
	{
	...
	// compute step-size, take care of decay
    _T computeBBStep() {
	...
	// compute inf-norm of chopped gradient
